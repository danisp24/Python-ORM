...
def find_authors_nationalities():
    ...
    return '\n'.join(str(f"{x.first_name} {x.last_name} is {x.nationality}") for x in authors_with_nationalities)

    # vtori variant po-chetim
...
def delete_review_by_id(review_id):
    ...
    # ako ne sme sigurni dali go ima v bazata taka handlevame greshkata koqto moje da ni dade
...
def filter_authors_by_birth_year(first_year, second_year):
    filtered_authors = (Author.objects.filter
                        (birth_date__year__gte=first_year,
                         birth_date__year__lte=second_year).
                        order_by('-birth_date'))
...
def change_reviewer_name(reviewer_name, new_name):
    (Review.objects.filter(reviewer_name=reviewer_name).
